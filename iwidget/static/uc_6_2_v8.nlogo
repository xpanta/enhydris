...
Based on this opinion each agent "decides" its water demand by choosing between the mean water demand of the population and a water demand reduced by a conservation rate selected by the modeller. This choice is non deterministic which means that agents with positive opinion regarding water conservation may eventually choose not to conserve water. In order to accomodate the increase of water demand due to the increasing quality of life conditions the population's mean monthly water demand is affected by an annual increase rate of 1%.  

Each household-agent is characterised by those parameters that shape the social impact exerted to their water conservation opinion by the other household-agent and by water policies such as awareness raising campaigns and water price changes. There is also a dummy variable that represents the parameters of a household-agent's water demand behaviour.
