...
globals [mean-water-demand-user total-water-demand Volatility_SocialImpact end-pricingpolicy Environmental_Awareness_Campaign price_change_info results1 r1 results2 r2]
turtles-own [Z water-demand conserv-dummy socio-dem influence_strength opinion_strength opinion-watercons Media_strength personal_strength Media_impact list1 network_strength impact Pnochange Pchange]
...
; Setup the household's characteristics
  ...
  ask turtles [set conserv-dummy random-normal 0.5 0.1 if conserv-dummy < 0 [set conserv-dummy 0] if conserv-dummy > 1 [set conserv-dummy 1]]
  ...
  ask turtles with [opinion-watercons = -1] [set water-demand mean-water-demand]
  ask turtles with [opinion-watercons = 1][
  set Z random-float 1
  ifelse conserv-dummy >= Z [set water-demand (1 - conservation-rate) * mean-water-demand][set water-demand mean-water-demand]]
  set total-water-demand mean [water-demand] of turtles  

  ask n-of (%_highest_demographic_class * population) turtles with [socio-dem = 10] [set socio-dem 4]
  ask n-of (%_middle_demographic_class * population) turtles with [socio-dem = 10] [set socio-dem 2]
  ask turtles with [socio-dem = 10] [set socio-dem 0]
...
to go
  ...
  social-influence
  calc-water-demand
  set r1 (count turtles with [opinion-watercons = 1] / count turtles)
  set results1 lput r1 results1
  set r2 total-water-demand
  set results2 lput r2 results2
  if ticks = total-time-of-simulation [output-print results1 output-print results2
      stop]
  tick
end  
...
to calc-social-distance
  ask links [set social-distance abs ([socio-dem] of end1 - [socio-dem] of end2)]
end
...
to calc-water-demand
  if ticks mod 12 = 0 [set mean-water-demand-user round (mean-water-demand * (1 + water-demand-increase-rate))]
ask turtles with [opinion-watercons = -1] [set water-demand mean-water-demand-user]
ask turtles with [opinion-watercons = 1][
  set Z random-float 1
  ifelse conserv-dummy >= Z [set water-demand (1 - conservation-rate) * mean-water-demand-user][set water-demand mean-water-demand-user]]
set total-water-demand mean [water-demand] of turtles  
end
@#$#@#$#@
GRAPHICS-WINDOW
...
1048
690
Mean monthly water demand in litres per person per day
Months
l/p/d
0.0
120.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy ticks total-water-demand"
...
@#$#@#$#@
## Support for assessing policy effects
...
Agent based modelling (ABM) is a promising tool for simulating the urban water system’s dynamics  and thus able to support urban water demand management strategies .  The “Support for assessing policy effects” model is built using NetLogo 5.0.3 a multi-agent programmable modelling environment . 

This model focuses on the urban population that resides or moves into a specific area.  The model does not include geographic information of the area and has a monthly time step. 
...
Household-agents make monthly decisions about their opinion regarding water conservation (positive (+1) / negative (-1)). These decisions are related to the social impact that is exerted to the household-agents by their social network (other household-agents) and by water policies.
...
Each household-agent is characterised by a set of parameters that shape the social impact and by a dummy variable that represents the parameters of a household-agent's water demand behaviour. The model's parameters are difficult to quantify, therefore, these variables are set based on previous studies (Wragg, 2006) . For this specific case: 
...
Following the decision of a positive or negative attitude towards water conservation each household- agent "decides" its water demand by choosing between the mean water demand of the population and a water demand reduced by a conservation rate selected by the modeller. This choice is non deterministic which means that agents with positive opinion regarding water conservation may eventually choose not to conserve water. In order to accommodate the increase of water demand due to the increasing quality of life conditions the population's mean monthly water demand is affected by an annual increase rate. 
...
Setup of water demand variables

1. Select from the slider "mean-water-demand" the mean monthly water demand in litres per person per day of the population under investigation (if such information is not available make an assumption based on the RBD's reported mean values or country's reported values).

2. Select from the slider "water-demand-increase-rate" an annual increase rate for the mean monthly water demand that is relevant to the population based on expert knowledge.

3. Select from the slider "conservation-rate" the percentage of decrease of the mean monthly water demand. Assign different values and of conservation rate and compare the results of the different experiments.
...
The results of the model are presented in the plots "% of positive opinions towards conservation" and "mean monthly water demand in litres per person per day"  

By right clicking the plots it is possible either to copy the image of the plot (right click the plot / click on COPY IMAGE) or to copy the data of the plot by creating a .csv file named as the plot (right click the plot / click on EXPORT).

The results are also exported in the Output window. By right clicking the window it is possible to copy the results and paste them, for further analysis, in a spreadsheet software.  
